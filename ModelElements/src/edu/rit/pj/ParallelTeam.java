...
// Exported constructors.
	...
	public ParallelTeam()
	 ...
	 public ParallelTeam (int K) { // Added by Paco
	   this("Unnamed",K);
	 }
	...
	public ParallelTeam
		...
		myThread[0] = new ParallelTeamThread_0 (name+" - 0",this, 0); // Modified by Paco
		for (int i = 1; i < K; ++ i)
			{
			myThread[i] = new ParallelTeamThread (name+" - "+i,this, i); // Modified by Paco
			}
//    for (int i = 0; i < K; ++ i) myThread[i].setName(name+" - "+i);  // Added by Paco
		
		}
	
  protected void finalize() throws Throwable { // Added by Paco
    try {
      terminateAll(); // terminate running threads
    } finally {
      super.finalize();
    }
  }
  
	public void terminateAll() { // Added by Paco
  ...
  public ParallelTeamThread[] getThreads() { // Added by Paco
    return myThread;
  }
	
//	public void setNumberOfThreads (String name, int threadsNumber) { Does not work correctly
//	  if (K<threadsNumber) {
//	    ParallelTeamThread[] oldThreads = myThread;
//	    myThread = new ParallelTeamThread [threadsNumber];
//	    for (int i=0; i<K; ++i) myThread[i] = oldThreads[i];
//	    for (int i=K; i<threadsNumber; ++i) myThread[i] = new ParallelTeamThread (this, i);
//	  }
//	  K = threadsNumber;
//    for (int i = 0; i < K; ++ i) myThread[i].setName(name + " - "+i);  // Added by Paco
//	}
...
// Exported operations.
	...
	public final void execute
		...
		try
			...
			// aborts the execute() method.
			if (myRegion!=null) myRegion.finish(); // null check added by Paco
			}

		finally
			{
			// Clean up.
			if (myRegion!=null) myRegion.myTeam = null; // null check added by Paco
			myExceptionMap = null;
			myRegion = null;
			}
		}
		...
		for (int i = 1; i < K; ++ i)
			...
			if (thread_i.myBarrierFlag != newBarrierFlag)
				...
				while (thread_i.myBarrierFlag != newBarrierFlag)
					{
				  if (thread_i.mustEnd) { // Added by Paco
//				    System.err.println ("Must terminate by barrier at Team "+thread_i.getName());
				    return;
				  }
					spinner.spin();
					}
