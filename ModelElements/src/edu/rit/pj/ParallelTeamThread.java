...
// Hidden data members.
	...
	volatile boolean mustEnd = false; // Added by Paco
...
// Exported constructors.
	...
	public ParallelTeamThread
		(String name, // Added by Paco
		 ParallelTeam theTeam,
		 int theIndex)
		{
		myTeam = theTeam;
		myIndex = theIndex;
		setName(name); // Added by Paco
		setDaemon (true);
		start();
		}

	public void terminate() { // Added by Paco
	  mustEnd = true;
	  myRegionBeginSemaphore.release();
	}
...
// Exported operations.
	...
	public void run()
		...
		for (;;)
			...
			if (mustEnd) { // Added by Paco
//		    System.err.println ("Must terminate "+getName());
			  return;
			}
	...
	void barrier()
		...
		if (myTeam.myBarrierFlag != newBarrierFlag)
			...
			while (myTeam.myBarrierFlag != newBarrierFlag)
				...
				spinner.spin();
				if (mustEnd) { // Added by Paco
//				  System.err.println ("Must terminate at barrier "+getName());
				  return;
				}
				}
			}
