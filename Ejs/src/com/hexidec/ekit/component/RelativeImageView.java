  ...
  private boolean      fGrowProportionally; // should grow be proportional?
	...
	private void initialize(Element elem)
	{
		...
		try
		{
			...
			@SuppressWarnings("unused")
      AttributeSet attribute = elem.getAttributes();
			if(isURL())
			{
	...
	*/
	...
	float getVerticalAlignment()
	{
		...
		if(align != null)
		{
			...
			if(align.equals(TOP_POS) || align.equals(TEXTTOP))
			{
				return 0.0f;
			}
			else if(align.equals(CENTER_POS) || align.equals(MIDDLE) || align.equals(ABSMIDDLE))
			{
				return 0.5f;
			}
		}
	...
	{
		AttributeSet attribute = fElement.getAttributes();
		if(attribute.isDefined(name))
		{
			int i;
			String val = (String)attribute.getAttribute(name);
			if(val == null)
			{
				i = iDefault;
			}
	...
	*/
	...
	protected void resize(int width, int height)
	{
		...
		attribute.addAttribute(HTML.Attribute.WIDTH ,Integer.toString(width));
		attribute.addAttribute(HTML.Attribute.HEIGHT,Integer.toString(height));
		((StyledDocument)getDocument()).setCharacterAttributes(fElement.getStartOffset(), fElement.getEndOffset(), attribute, false);
	}
